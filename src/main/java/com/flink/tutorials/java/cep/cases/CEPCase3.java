...
package com.flink.tutorials.java.cep.cases;
...
public class CEPCase3 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // ------ Create pattern "b+ c" ------
        Pattern<Event, ?> pattern = Pattern.<Event>begin("start", skipStrategy).where(new SimpleCondition<Event>() {
            @Override
            public boolean filter(Event value) {
                return value.getName().startsWith("b");
            }
        }).oneOrMore().consecutive().next("next").where(new SimpleCondition<Event>() {
            @Override
            public boolean filter(Event value) {
